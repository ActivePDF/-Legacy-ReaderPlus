...
<%@ Page Language="VB" AutoEventWireup="true" CodeFile="Prepopulate_Form.aspx.vb" Inherits="Prepopulate_Form" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
    <head runat="server">
        <title>DocSpace&#x2122; Reader Plus Prepopulate Form Sample</title>
        ...
        <script>
            window.onload = function () {
                ...
                readerplus.Document.addEventListener("load", function () {
                    // Populate form field prior to opening
                    var page = readerplus.Document.getPage(1);
                    var fields = page.getAllFields();
                    for (var i = 0; i < fields.length; i++) {
                        // first name
                        if (fields[i].name === 'topmostSubform[0].Page1[0].f1_04[0]') {
                            var field = readerplus.Document.getField(fields[i].id);
                            field.setFieldProperties({ value: "John" });
                        }
                        // last name
                        if (fields[i].name === 'topmostSubform[0].Page1[0].f1_05[0]') {
                            var field = readerplus.Document.getField(fields[i].id);
                            field.setFieldProperties({ value: "Doe" });
                        }
                    }
                ...
                $.ajax({
                    type: "POST",
                    url: "Prepopulate_form.aspx/GetPDFData",
                    data: "{}",
                    dataType: "json",
